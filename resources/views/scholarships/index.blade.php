            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    @can('scholarship-create')
                        <h3><a href="{{ route('scholarship.create') }}" class="btn btn-outline btn-info">+
                                {{ __('Add New Application') }}</a></h3>
                    @endcan
                </div>
                <div class="col-sm-6">
                                    ...
                                    <div class="col-sm-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="application_no" class="form-control"
                                                value="{{ request()->application_no }}" placeholder="@lang('Application No')">
                                        </div>
                                    </div>
                                    <div class="col-sm-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="year" id="year">
                                                ...
                                                @foreach ($years as $key => $value)
                                                    <option value="{{ $key }}"
                                                        {{ old('year', request()->year) == $key ? 'selected' : '' }}>
                                                        {{ $value }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control ambitious-form-loading" name="school_or_college"
                                                id="school_or_college">
                                                <option value="1"
                                                    {{ old('school_or_college', request()->school_or_college) == '1' ? 'selected' : '' }}>
                                                    @lang('School')</option>
                                                <option value="2"
                                                    {{ old('school_or_college', request()->school_or_college) == '2' ? 'selected' : '' }}>
                                                    @lang('College')</option>
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="scholarship_school_id"
                                                ...
                                                @foreach ($schools as $key => $value)
                                                    <option value="{{ $key }}"
                                                        {{ old('scholarship_school_id', request()->scholarship_school_id) == $key ? 'selected' : '' }}>
                                                        {{ $value }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="scholarship_college_id"
                                                ...
                                                @foreach ($colleges as $key => $value)
                                                    <option value="{{ $key }}"
                                                        {{ old('scholarship_college_id', request()->scholarship_college_id) == $key ? 'selected' : '' }}>
                                                        {{ $value }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                        ...
                        <tbody>
                            @foreach ($scholarships as $scholarship)
                                <tr>
                                        ...
                                        <td>
                                            @if ($scholarship->year != date('Y'))
                                                <a href="" class="btn btn-info"
                                                    data-href="{{ route('scholarship.renewal', $scholarship) }}"
                                                    data-toggle="modal" data-target="#myModalRenew">Renew</a>
                                            @endif
                                        </td>
                                    @endcanany
                                    @canany(['scholarship-read', 'scholarship-update', 'scholarship-delete'])
                                    ...
                                    @endcan  --}}
                                            <a href="{{ route('scholarship.download', $scholarship) }}" target="_blank"
                                                class="btn btn-info btn-outline btn-circle btn-lg" data-toggle="tooltip"
                                                title="download"><i
                                                    class="fa fa-download ambitious-padding-btn"></i></a>&nbsp;&nbsp;

                                            @can('scholarship-read')
                                                <a href="{{ route('scholarship.show', $scholarship) }}"
                                                    class="btn btn-info btn-outline btn-circle btn-lg" data-toggle="tooltip"
                                                    title="show"><i class="fa fa-eye ambitious-padding-btn"></i></a>&nbsp;&nbsp;
                                            @endcan
                                            @can('scholarship-update')
                                                <a href="{{ route('scholarship.edit', $scholarship) }}"
                                                    class="btn btn-info btn-outline btn-circle btn-lg" data-toggle="tooltip"
                                                    title="Edit"><i
                                                        class="fa fa-edit ambitious-padding-btn"></i></a>&nbsp;&nbsp;
                                            @endcan
                                            @can('scholarship-delete')
                                                <a href="#" data-href="{{ route('scholarship.destroy', $scholarship) }}"
                                                    class="btn btn-info btn-outline btn-circle btn-lg" data-toggle="modal"
                                                    data-target="#myModal" title="Delete"><i
    ...
    <script>
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete this record?")) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route('scholarship.destroy', ':id') }}'.replace(':id', id),
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
